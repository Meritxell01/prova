...
class Provider extends Admin_Controller {
    ...
    public function form() {
        ...
        if (!$this->mdl_provider->validate()) {
            ...
            if (!$_POST AND uri_assoc('provider_id',4)) {
                $this->mdl_provider->prep_validation(uri_assoc('provider_id',4));
            }
            
            $this->load->view('add_provider');
        
        } else {
            $this->mdl_provider->save();
    ...
    public function delete() {
        ...
        if (uri_assoc('provider_id',4)) {
            
            $this->mdl_provider->delete(array('provider_id' => uri_assoc('provider_id',4)));
