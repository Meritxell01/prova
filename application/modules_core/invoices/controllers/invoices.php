...
class Invoices extends Admin_Controller {
    ...
    function index() {
		...
		$delivery_note_number = uri_assoc('delivery_note_number');
		...
		if($is_quote==1){
			 $params['where']['mcb_invoices.invoice_group_id'] = 3;
		}else if($delivery_note_number==2){
			 $params['where']['mcb_invoices.invoice_group_id'] = 5;
		}else{
    ...
    function create() {
        ...
        if (!$this->mdl_invoices->validate_create()) {
            ...
            else if (uri_assoc('client_id', 4) and !$_POST) {

                $this->mdl_invoices->set_form_value('client_id', uri_assoc('client_id', 4));

            }else if (uri_assoc('client_id', 5) and !$_POST) {

                $this->mdl_invoices->set_form_value('client_id', uri_assoc('client_id', 5));
    ...
    function edit() {
        ...
        $contact_params = array(
            'where'	=>	array(
                'mcb_contacts.client_id'    =>  $invoice->client_id
            )
        );
        ...
        $data = array(
            ...
            'invoice_categories' => $this->mdl_expense_category->get(),
            'tab_index'			=>	$tab_index,
    ...
    function _post_handler() {
        ...
        } elseif ($this->input->post('btn_add_delivery_note_number')) {

            redirect('invoices/create/delivery_note_number/2');

        }

        elseif ($this->input->post('btn_add_quote')) {

            redirect('invoices/create/quote/1');
