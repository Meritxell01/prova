...
class Mdl_Invoices extends MY_Model {
    ...
    public function __construct() {
		...
		mcb_invoices.*,
		mcb_invoice_amounts.*,
                mcb_contacts.first_name AS contact_first_name,
                mcb_contacts.last_name AS contact_last_name,
        mcb_clients.*,
        mcb_payment_methods.*,
        mcb_expense_method.*,
		mcb_users.username,
        ...
        $this->joins = array(
            ...
            'mcb_expense_method'  =>  array(
                'mcb_expense_method.expense_category_id = mcb_invoices.invoice_category_id',
                'left'
            ),
            'mcb_users'				=>	array(
            ...
            'mcb_payment_methods'  =>  array(
                'mcb_payment_methods.payment_method_id = mcb_invoices.payment_method_id',
                'left'
            ),
            'mcb_contacts'              =>      array(
    ...
    public function get_recent_open($limit = 10) {
        ...
        $params = array(
            'limit'	=>	$limit,
            'where'	=>	array(
                'invoice_status_type'	=>	$this->mdl_mcb_data->setting('default_open_status_id'),
                'invoice_is_quote'		=>	0
            ),
            'having'	=>	array(
    ...
    public function get_recent_closed($limit = 10) {
        ...
        $params = array(
            'limit'	=>	$limit,
            'where'	=>	array(
                'invoice_status_type'	=>	$this->mdl_mcb_data->setting('default_closed_status_id'),
                'invoice_is_quote'		=>	0
            )
        );
    ...
    public function save($client_id, $date_entered, $invoice_is_quote = 0, $strtotime = TRUE, $custom_fields = array()) {
...
die();
	...
	// input default value of custom fields if any
	if ($custom_fields) {
		foreach ($custom_fields as $a_field) {
			$db_array[$a_field->column_name] = $a_field->field_default_value;
		}
	}
    ...
    public function save_invoice_options($custom_fields = NULL) {
        ...
        $db_array = array(
            ...
            'invoice_category_id'       =>  $this->input->post('invoice_category_id'),
        );
...
//Oscar: Funció set_invoice_status_id

    public function set_invoice_status_id($invoice_id, $invoice_status_id) {

        $this->db->where('invoice_id', $invoice_id);

        $this->db->set('invoice_status_id', $invoice_status_id);
    ...
    public function validate() {
        ...
        $this->form_validation->set_rules('invoice_status_id', $this->lang->line('paid'));
    ...
    public function set_invoice_additional($invoice, $params = NULL) {
        ...
        if (isset($params['get_invoice_notes'])) {

            $invoice->invoice_tags = $this->get_invoice_notes($invoice->invoice_id);
    ...
    public function get_invoice_notes($invoice_id) {
	...
	public function getAlbara(){
	
	$sql= $this->db->query("select * from mcb_invoices mi
	inner join mcb_clients mc on mi.client_id = mc.client_id
	inner join mcb_invoice_statuses mcs on mi.invoice_status_id = mcs.invoice_status_id
	where invoice_group_id=5");
	return $sql->result();
	}
