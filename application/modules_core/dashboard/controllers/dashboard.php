...
class Dashboard extends Admin_Controller {
	...
	function index() {
		...
		if ($this->mdl_mcb_data->setting('dashboard_show_open_invoices') == 'TRUE') {
			...
			$data['open_invoices'] = $this->mdl_invoices->get_recent_open(10000);

		}

		if ($this->mdl_mcb_data->setting('dashboard_show_pending_invoices') == 'TRUE') {

			$data['pending_invoices'] = $this->mdl_invoices->get_recent_pending(10000);

		}

		if ($this->mdl_mcb_data->setting('dashboard_show_closed_invoices') == 'TRUE') {

			$data['closed_invoices'] = $this->mdl_invoices->get_recent_closed(10000);

		}

		if ($this->mdl_mcb_data->setting('dashboard_show_overdue_invoices') == 'TRUE') {

			$data['overdue_invoices'] = $this->mdl_invoices->get_recent_overdue(10000);

		}
                
                // get client list
                $data['client_total'] = $this->mdl_clients->count_clients();
